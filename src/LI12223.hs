...
module LI12223 (
  ...
  -- ** Menu
  Menu(..), SubMenu(..), Opcao(..), ModoJogo(..), Niveis(..), Acao(..)
  ) where
...
data Obstaculo
  ...
  | Parede -- ^ as paredes que delimitam areas em 'Loja'
  | Portal -- ^ os portais que permitem a entrada nos menus em 'Loja'
  deriving (Show, Read, Eq)
...
-- | Construtores que permitem criar o Menu.
data Niveis = NadaNiveis
            | VoltarNiveis
            | Nivel1 Int
            | Nivel2 Int
            | Nivel3 Int
...
data Menu = Opcoes Opcao
            ...
            | InicioMulti
            | PerdeuMulti Int
            | InicioCarreira Niveis Acao
            | PerdeuCarreira
            | InicioPersonalizar SubMenu
            | FimPersonalizar
            deriving (Show, Read, Eq)
